...
// .wrangler/tmp/pages-uM6BLg/functionsWorker-0.5026519353871641.mjs
  ...
  const response = await fetch("https://api.notion.com/v1/pages", {
    ...
    body: JSON.stringify({
      ...
      properties: {
        ...
        "Neg\xF3cio": {
          rich_text: lead.business ? [{ text: { content: lead.business } }] : []
        },
        "Email": {
          email: lead.email
        },
...
__name(createNotionLead, "createNotionLead");
__name2(createNotionLead, "createNotionLead");
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: `
        ...
        <p><strong>Neg\xF3cio:</strong> ${escapeHtml(lead.business || "-")}</p>
        <p><strong>Email:</strong> ${escapeHtml(lead.email)}</p>
        <p><strong>Telefone:</strong> ${escapeHtml(lead.phone || "-")}</p>
        <p><strong>Servi\xE7o:</strong> ${escapeHtml(lead.service)}</p>
...
__name2(sendNotificationEmail, "sendNotificationEmail");
...
async function onRequestPost(context) {
  ...
  try {
    ...
    if (name.trim().length < 2) {
      return json({ error: "Nome inv\xE1lido." }, 400);
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json({ error: "Email inv\xE1lido." }, 400);
    }
    if (!service.trim()) {
      return json({ error: "Servi\xE7o obrigat\xF3rio." }, 400);
    }
    if (message.trim().length < 20) {
      return json({ error: "Mensagem demasiado curta." }, 400);
    }
    if (!turnstileToken) {
      return json({ error: "Verifica\xE7\xE3o anti-bot em falta." }, 400);
    }
    ...
    if (!turnstileResult.success) {
      return json(
        {
          error: "Falha na verifica\xE7\xE3o anti-bot.",
...
// .wrangler/tmp/bundle-CxXBOC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CxXBOC/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5026519353871641.js.map
